...
namespace Stew {
	...
	public class Cookbook : Object {
		...
		public void configure (string[] args) throws CookbookError {
			...
			var conf_data = "# This file is automatically generated by the Stew configure stage\n";
		...
		public void load () throws CookbookError, RecipeError {
			...
			try {
				...
				if (e is FileError.NOENT) {
					conf_file = new Stew.Recipe (pretty_print);
				} else {
		...
		private void find_toplevel () throws CookbookError {
				...
				try {
					...
					if (e is FileError.NOENT) {
						if (have_recipe) {
							...
							throw new CookbookError.NO_RECIPE ("No recipe found.\nTo build a project Stew requires a file called 'Recipe' in the current directory.");
						}
			...
			var minimum_stew_version = t.get_variable ("project.minimum-stew-version");
			if (minimum_stew_version != null && pkg_compare_version (VERSION, minimum_stew_version) < 0) {
				throw new CookbookError.TOO_OLD ("This version of Stew is too old for this project.\nVersion %s or greater is required.\nThis is Stew %s.".printf (minimum_stew_version, VERSION));
			}
		...
		private void generate_template_rules (Template template) throws Error {
			...
			foreach (var entry in template.get_tagged_list ("files")) {
				...
				var command = "@stew-template %s %s".printf (template_file, file);
				if (variables != null) {
		...
		private void generate_program_rules (Program program) throws Error {
			...
			if (buildable_modules.length () > 0) {
				...
				foreach (var test in program.test_names) {
					...
					recipe.test_rule.add_command ("@stew-test run %s %s".printf (results_filename, command));
				}
			} else {
				var rule = recipe.add_rule ();

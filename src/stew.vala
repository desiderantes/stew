...
public class StewApp {
	...
	public static int main (string[] args) {
		...
		var cookbook = new Stew.Cookbook (original_dir, pretty_print);
		...
		if (do_list_targets) {
			...
			var build_dir = "%s/".printf (Stew.get_relative_path (cookbook.current_recipe.dirname, cookbook.current_recipe.build_directory));
		...
		foreach (var target in targets) {
			...
			Stew.BuilderFlags flags = 0;
			if (pretty_print) {
				flags |= Stew.BuilderFlags.PRETTY_PRINT;
			}
			if (debug_enabled) {
				flags |= Stew.BuilderFlags.DEBUG;
			}
			if (do_parallel) {
				flags |= Stew.BuilderFlags.PARALLEL;
			}
			var builder = new Stew.Builder (original_dir, flags);
			...
			builder.build_target.begin (cookbook.current_recipe, Stew.join_relative_dir (cookbook.current_recipe.dirname, target), 
				(o, x) => {
				n_remaining--;
					try {

public class TestStewGetSymbols
{
    public static int main (string[] args)
    {
        ...
        var command = "../../../src/stew-get-symbols %s".printf (test_args);
        string output;
        int exit_status;
        try
        {
        ...
        if (Process.if_exited (exit_status))
        {
            ...
            {
                stderr.printf ("stew-get-symbols returned with exit code %d\n", Process.exit_status (exit_status));
                return Posix.EXIT_FAILURE;
            }
        }
        else
        {
             stderr.printf ("stew-get-symbols exited with signal %d\n", Process.term_sig (exit_status));
             return Posix.EXIT_FAILURE;
        }

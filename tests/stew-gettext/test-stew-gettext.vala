public class TestStewGettext {
	public static int main (string[] args) {
		...
		var command = "../../../src/stew-gettext --domain test-domain --mime-type %s %s".printf (mime_type, file_to_translate);
		string output;
		int exit_status;
		try {
			string[] argv;
		...
		if (Process.if_exited (exit_status)) {
			if (Process.exit_status (exit_status) != 0) {
				stderr.printf ("stew-gettext returned with exit code %d\n", Process.exit_status (exit_status));
				return Posix.EXIT_FAILURE;
			}
		} else {
			 stderr.printf ("stew-gettext exited with signal %d\n", Process.term_sig (exit_status));
			 return Posix.EXIT_FAILURE;
		}

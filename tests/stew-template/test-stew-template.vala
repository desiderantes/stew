public class TestStewTemplate {
	public static int main (string[] args) {
		...
		var output_filename = Path.build_filename (Environment.get_tmp_dir (), "stew-template-test-XXXXXX");
		FileUtils.mkstemp (output_filename);
		var command = "../../../src/stew-template %s %s %s".printf (file_to_translate, output_filename, variables);
		int exit_status;
		try {
			string[] argv;
		...
		if (Process.if_exited (exit_status)) {
			if (Process.exit_status (exit_status) != 0) {
				stderr.printf ("stew-template returned with exit code %d\n", Process.exit_status (exit_status));
				return Posix.EXIT_FAILURE;
			}
		} else {
			 stderr.printf ("stew-template exited with signal %d\n", Process.term_sig (exit_status));
			 return Posix.EXIT_FAILURE;
		}
